...
def CNN(weights_path=None):
    cnn = Sequential()
    cnn.add(Convolution2D(64, (5, 5), padding="same", activation="relu", input_shape=(1, 128, 128)))
    ...
    cnn.add(Dropout(0.5))
    cnn.add(Dense(300, activation="relu", name='fc5'))
    cnn.add(Dropout(0.5))
    cnn.add(Dense(7, activation="softmax", name='fc6'))
